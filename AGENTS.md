...
- Discover plausible Codex CLI installations from `PATH`, common Homebrew locations, and bundled Codex/ChatGPT app resources. Show each binary's source and reported version, persist an explicit absolute selection, and retain custom-path support. Never silently switch binaries based on `PATH` order when multiple installations exist.
